...
# Causal attention: FA3 > FA2 > PyTorch SDPA. All take (B,T,H,D) layout.
try:
    from flash_attn_interface import flash_attn_func as _fa_func
    def causal_attention(q, k, v):
        return _fa_func(q, k, v, causal=True)
except ImportError:
    try:
...
class Hyperparameters:
    ...
    ttt_chunk_tokens = int(os.environ.get("TTT_CHUNK_TOKENS", 2097152))  # 2M tokens per chunk
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for i, m in enumerate(self._bank_meta):
                ...
                # Muon+ row-wise normalization (arxiv:2602.21545) — F.normalize for fused kernel
...
def eval_val(
    ...
    with torch.inference_mode():
        for batch_seq_start in range(seq_start, seq_end, local_batch_seqs):
            ...
            batch_idx += 1
            if log_fn and total_batches > 1 and (batch_idx % max(total_batches // 3, 1) == 0 or batch_idx == total_batches):
                log_fn(f"  eval progress: {batch_idx}/{total_batches} batches, running_loss:{(val_loss_sum / max(val_token_count, 1)).item():.4f}")
...
def _vram_log(device: torch.device, label: str, log_fn=None):
    """Log peak VRAM usage and reset the counter."""
    peak_mb = torch.cuda.max_memory_allocated(device) / 1e6
    alloc_mb = torch.cuda.memory_allocated(device) / 1e6
    free_mb = (torch.cuda.get_device_properties(device).total_memory - torch.cuda.memory_allocated(device)) / 1e6
    msg = f"vram:{label} peak:{peak_mb:.0f}MB alloc:{alloc_mb:.0f}MB free:{free_mb:.0f}MB"
    if log_fn:
        log_fn(msg)
    torch.cuda.reset_peak_memory_stats(device)

def _prebuild_windows(window_starts: list[int], seq_len: int, val_tokens: Tensor, device: torch.device):
    """Vectorized window construction — one gather op instead of per-window Python loop."""
    num_win = len(window_starts)
    if num_win == 0:
        return torch.zeros(0, seq_len, dtype=torch.int64, device=device), \
               torch.zeros(0, seq_len, dtype=torch.int64, device=device), \
               torch.zeros(num_win, dtype=torch.int64, device=device)
    total_tokens = val_tokens.numel() - 1
    # Build indices on CPU (same device as val_tokens), gather, then move to GPU
    # val_tokens is uint16 which doesn't support fancy indexing on CPU — cast to long first
    starts = torch.tensor(window_starts, dtype=torch.long)
    offsets = torch.arange(seq_len + 1, dtype=torch.long)
    indices = starts.unsqueeze(1) + offsets.unsqueeze(0)  # (num_win, seq_len+1)
    indices = indices.clamp(max=val_tokens.numel() - 1)
    all_data = val_tokens.long()[indices]  # (num_win, seq_len+1)
    all_x = all_data[:, :-1].contiguous().to(device)
    all_y = all_data[:, 1:].contiguous().to(device)
    # Compute effective window lengths
    ends = (starts + seq_len).clamp(max=total_tokens)
    wlens = (ends - starts).to(device)  # (num_win,)
    return all_x, all_y, wlens
...
def eval_val_sliding(
    ...
    # Pre-build all window tensors in one vectorized gather
    all_x, all_y, all_wlens = _prebuild_windows(my_windows, seq_len, val_tokens, device)
    num_win = len(my_windows)
    if log_fn:
        log_fn(f"  sliding_eval: {num_win} windows, batch_seqs={batch_seqs}")
    loss_sum = torch.zeros((), device=device, dtype=torch.float64)
    token_count = torch.zeros((), device=device, dtype=torch.float64)
    byte_count = torch.zeros((), device=device, dtype=torch.float64)
    base_model.eval()
    compiled_logits = torch.compile(base_model.forward_logits, dynamic=False, fullgraph=True)
    total_slide_batches = (num_win + batch_seqs - 1) // batch_seqs
    slide_batch_idx = 0
    with torch.inference_mode():
        for bi in range(0, num_win, batch_seqs):
            be = min(bi + batch_seqs, num_win)
            x_batch = all_x[bi:be]
            y_batch = all_y[bi:be]
            bsz = be - bi
            ...
            for i in range(bsz):
                ...
                byte_count += tb.sum()
            slide_batch_idx += 1
            if log_fn and total_slide_batches > 1 and (slide_batch_idx % max(total_slide_batches // 3, 1) == 0 or slide_batch_idx == total_slide_batches):
                log_fn(f"  sliding_eval progress: {slide_batch_idx}/{total_slide_batches} batches")
    if dist.is_available() and dist.is_initialized():
...
def eval_val_ttt(
    ...
    stride: int,
    batch_seqs: int = 1024,
    train_batch_seqs: int = 128,
    eval_seq_len: int | None = None,
    log_fn=None,
) -> tuple[float, float]:
    """Legal score-first TTT: score each chunk under no_grad, then train on already-scored tokens."""
    seq_len = eval_seq_len or args.train_seq_len
    total_tokens = val_tokens.numel() - 1
    ttt_chunk = args.ttt_chunk_tokens
    context_size = seq_len - stride

    # Build sliding windows and assign each to a chunk based on scored-token start
    window_starts = [ws for ws in range(0, total_tokens, stride)
    ...
    total_windows_all = len(window_starts)
    if log_fn:
        log_fn(f"ttt: start chunks={num_chunks} ttt_lr={args.ttt_lr} "
               f"ttt_epochs={args.ttt_epochs} chunk_tokens={ttt_chunk} "
               f"batch_seqs={batch_seqs} train_batch_seqs={train_batch_seqs} "
               f"total_windows={total_windows_all} windows_per_gpu={total_windows_all // max(world_size, 1)}")

    compiled_logits = torch.compile(base_model.forward_logits, dynamic=False, fullgraph=True)
    # Warmup torch.compile with a dummy forward to trigger JIT compilation
    if log_fn:
        log_fn("ttt: warming up torch.compile (this takes 1-3 min)...")
    t_warmup = time.perf_counter()
    base_model.eval()
    with torch.no_grad():
        dummy_x = torch.zeros(1, seq_len, dtype=torch.int64, device=device)
        with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
            _ = compiled_logits(dummy_x)
        del dummy_x
    torch.cuda.synchronize()
    if log_fn:
        log_fn(f"ttt: compile warmup done in {time.perf_counter() - t_warmup:.1f}s")

    loss_sum = torch.zeros((), device=device, dtype=torch.float64)
    token_count = torch.zeros((), device=device, dtype=torch.float64)
    byte_count = torch.zeros((), device=device, dtype=torch.float64)

    # Setup SGD optimizer for TTT
    ...
    for ci in range(num_chunks):
        ...
        # --- Phase 1: SCORE this chunk's windows under no_grad (scores are FINAL) ---
        my_s = len(windows) * rank // world_size
        my_e = len(windows) * (rank + 1) // world_size
        my_windows = windows[my_s:my_e]
        # Vectorized window construction for this chunk
        all_x, all_y, all_wlens = _prebuild_windows(my_windows, seq_len, val_tokens, device)
        num_win = len(my_windows)
        num_score_batches = (num_win + batch_seqs - 1) // batch_seqs
        if log_fn and ci == 0:
            log_fn(f"  ttt: first chunk: {num_win} windows/gpu, {num_score_batches} score batches")
        base_model.eval()
        with torch.no_grad():
            for bi in range(0, num_win, batch_seqs):
                be = min(bi + batch_seqs, num_win)
                ...
                for i in range(bsz):
                    ...
                    tb += (has_leading_space_lut[tgt] & ~is_boundary_token_lut[prev]).to(torch.float64)
                    byte_count += tb.sum()
        del all_x, all_y, all_wlens  # free scoring tensors before training
        t_score_done = time.perf_counter()

        # --- Phase 2: TRAIN on already-scored chunk tokens (skip last chunk) ---
        ...
        if not is_last_chunk and args.ttt_epochs > 0:
            ...
            if chunk_seqs > 0:
                ...
                for _ep in range(args.ttt_epochs):
                    for bs in range(0, my_chunk_seqs, train_batch_seqs):
                        be = min(bs + train_batch_seqs, my_chunk_seqs)
        ...
        if log_fn and (ci % max(num_chunks // 10, 1) == 0 or ci == num_chunks - 1):
            ...
            running_loss = (loss_sum / max(token_count, 1)).item()
            running_bpb = running_loss / math.log(2.0) * (token_count / max(byte_count, 1)).item() if token_count.item() > 0 else 0
            gpu_mem_mb = torch.cuda.max_memory_allocated() // 1024 // 1024
            chunk_time = t_chunk_done - t_chunk
            score_time = t_score_done - t_chunk
            train_time = t_chunk_done - t_score_done
            eta = elapsed / (ci + 1) * (num_chunks - ci - 1)
            log_fn(f"  ttt: chunk {ci+1}/{num_chunks} windows={num_win} score_batches={num_score_batches} "
                   f"train_steps={train_steps} score:{score_time:.1f}s train:{train_time:.1f}s "
                   f"chunk:{chunk_time:.1f}s running_bpb:{running_bpb:.4f} "
                   f"peak_mem:{gpu_mem_mb}MB elapsed:{elapsed:.0f}s eta:{eta:.0f}s")
...
def main() -> None:
    ...
    # --- EVAL_ONLY mode: skip training+GPTQ, load from existing final_model.int6.ptz ---
    eval_only = bool(int(os.environ.get("EVAL_ONLY", "1")))
    if eval_only:
        log0("eval_only:loading from final_model.int6.ptz")
        # Build a template model to get state dict shapes for dequantization
        template_model = GPT(
        ...
        unbanked_sd = _unbank_state_dict(template_sd, args.num_layers)
        del template_model
        # Load and dequantize
        ...
        log0(f"eval_only:dequantized, building eval model")
        ...
        log0("eval_only:model loaded, starting roundtrip eval")
        ...
        q_val_loss, q_val_bpb = eval_val(
            ...
            eval_seq_len=effective_eval_seq_len, log_fn=log0,
        )
        torch.cuda.synchronize()
        ...
        if args.ttt_enabled and args.eval_stride > 0:
            ...
            log0("ttt: loaded fresh model from quantized artifact")
            torch.cuda.synchronize()
            ...
            del ttt_model
        # Also run plain sliding eval as diagnostic
        elif args.eval_stride > 0 and args.eval_stride < sw_seq_len:
            torch.cuda.synchronize()
    ...
    while True:
        ...
        if should_log_train:
            toks_per_sec = step * args.train_batch_tokens / (approx_training_time_ms / 1000.0) if approx_training_time_ms > 0 else 0
            gpu_mem_mb = torch.cuda.max_memory_allocated() // 1024 // 1024
            log0(
                f"step:{step}/{args.iterations} train_loss:{train_loss.item():.4f} "
                f"lr_scale:{scale:.4f} muon_mom:{muon_momentum:.4f} "
                f"train_time:{approx_training_time_ms:.0f}ms step_avg:{approx_training_time_ms / step:.2f}ms "
                f"toks/s:{toks_per_sec:.0f} peak_mem:{gpu_mem_mb}MB"
            )
    ...
    _vram_log(device, "post_training", log_fn=log0)
    ...
    if args.lawa_enabled and len(lawa_queue) > 1:
        ...
        log0("weight_averaging:none")
    # Skip redundant diagnostic eval when no weight averaging was applied
    if ema_state is not None or (args.lawa_enabled and len(lawa_queue) > 1):
        torch.cuda.synchronize()
        ...
        torch.cuda.synchronize()
        log0(
            f"DIAGNOSTIC post_avg val_loss:{diag_val_loss:.4f} val_bpb:{diag_val_bpb:.4f} "
    ...
    log0(f"gptq:unbanked {len(unbanked_sd)} tensors")
    ...
    # Collect Hessians using training data (fast, no autoregressive generation)
    log0(f"gptq:collecting hessians from training data (64 batches)...")
    t_gen = time.perf_counter()
    hessians = collect_hessians(hessian_model, train_loader, args, device, grad_accum_steps, num_batches=64)
    log0(f"gptq:collected hessians for {len(hessians)} layers in {time.perf_counter()-t_gen:.1f}s")
    del hessian_model
    torch.cuda.empty_cache()
    quant_result, quant_meta = mixed_quantize_int6(unbanked_sd, {"mlp", "attn"}, hessians=hessians)
    log0(f"gptq:quantized {len(quant_result)} tensors to int6")
    quant_buf = io.BytesIO()
    torch.save({"w": quant_result, "m": quant_meta}, quant_buf)
    quant_raw = quant_buf.getvalue()
    log0(f"gptq:compressing {len(quant_raw)} bytes with lzma...")
    t_compress = time.perf_counter()
    quant_blob = lzma.compress(quant_raw, preset=9)
    log0(f"gptq:compressed to {len(quant_blob)} bytes in {time.perf_counter()-t_compress:.1f}s")
    if master_process:
    ...
    _vram_log(device, "post_gptq", log_fn=log0)
    if distributed:
        dist.barrier()
    with open("final_model.int6.ptz", "rb") as f:
        quant_blob_disk = f.read()
    ...
    restore_low_dim_params_to_fp32(eval_model)
    eval_model.load_state_dict(deq_state, strict=True)
    log0(f"gptq:roundtrip eval model loaded, starting evaluation...")
    compiled_eval = torch.compile(eval_model, dynamic=False, fullgraph=True)
    torch.cuda.synchronize()
    t_qeval = time.perf_counter()
    q_val_loss, q_val_bpb = eval_val(
    ...
    log0(f"final_int6_roundtrip_exact val_loss:{q_val_loss:.8f} val_bpb:{q_val_bpb:.8f}")
    _vram_log(device, "post_roundtrip_eval", log_fn=log0)
    sw_seq_len = effective_eval_seq_len
    # Skip plain sliding eval when TTT is enabled — TTT produces the final metric
    if not args.ttt_enabled and args.eval_stride > 0 and args.eval_stride < sw_seq_len:
        torch.cuda.synchronize()
        t_slide = time.perf_counter()
        sw_val_loss, sw_val_bpb = eval_val_sliding(
            args, eval_model, rank, world_size, device,
        ...
        _vram_log(device, "post_sliding_eval", log_fn=log0)
    # TTT eval: legal score-first test-time training
    if args.ttt_enabled and args.eval_stride > 0:
        # Load fresh model from quantized artifact for TTT (TTT modifies weights)
        del eval_model
        torch._dynamo.reset()
        torch.cuda.empty_cache()
        ttt_model = GPT(
        ...
        log0(f"final_ttt val_loss:{ttt_val_loss:.4f} val_bpb:{ttt_val_bpb:.4f} "
             ...
             f"time:{1000*(time.perf_counter()-t_ttt):.0f}ms")
        log0(f"final_ttt_exact val_loss:{ttt_val_loss:.8f} val_bpb:{ttt_val_bpb:.8f}")
        _vram_log(device, "post_ttt", log_fn=log0)
        del ttt_model
    if distributed:
        dist.destroy_process_group()
